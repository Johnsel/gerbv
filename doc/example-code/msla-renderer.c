...
example_callbacks_drawingarea_key_press_event (GtkWidget *widget, GdkEventKey *event)
{
	switch(event->keyval) {
		...
		case GDK_Down:
			...
			break;
		case GDK_Left:
			screenRenderInfo.lowerLeftX += 0.01;
			break;
		case GDK_Right:
			screenRenderInfo.lowerLeftX -= 0.01;
			break;
		...
		case GDK_z:
			/* notice the lower left corner doesn't move with this method...
			   ...
			   does it */
			screenRenderInfo.scaleFactorX += 1;
			screenRenderInfo.scaleFactorY += 1;
			break;
		case GDK_Z:
			screenRenderInfo.scaleFactorX -= 1;
			screenRenderInfo.scaleFactorY -= 1;
		...
		case GDK_s:
			printf("test");
			printf("%lf", screenRenderInfo.lowerLeftX);
...
main(int argc, char *argv[]) {
	...
	gerbv_open_layer_from_filename (mainProject, "top.gbr");
	...
	if ((mainProject->file[0] == NULL))
		...
		/* change the color of the first image (file[0]) to white */
	GdkColor whiteColor = {0, 0, 0, 0};
	mainProject->file[0]->color = whiteColor;
